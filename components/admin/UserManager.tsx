...
interface Props { clientId: string; users: Profile[] }

export default function UserManager({ clientId, users: initial }: Props) {
  ...
  async function inviteUser() {
    setLoading(true)
    setMsg('')
    // Create user via Supabase Admin (requires service role - use API route in production)
    ...
    if (data.user) {
      // Update profile with client_id and name
      ...
      setUsers(prev => [...prev, { id: data.user!.id, email: form.email, name: form.name, role: 'client', client_id: clientId }])
      setMsg('Usuário criado! Eles receberão um email de confirmação.')
    ...
    <div className="card p-0 overflow-hidden">
      ...
      {showForm && (
        <div className="p-5 bg-gray-50 border-b border-gray-100">
          <p className="text-sm font-medium text-gray-700 mb-3">Novo usuário cliente</p>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input className="input" type="email" placeholder="email@cliente.com" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
            </div>
            <div>
              <label className="label">Senha inicial</label>
              <input className="input" type="password" placeholder="Mínimo 6 caracteres" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} />
            </div>
          </div>
          {msg && <p className={`text-xs mt-2 px-3 py-2 rounded-lg ${msg.startsWith('Erro') ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-700'}`}>{msg}</p>}
          <div className="flex gap-2 mt-3">
            <button onClick={inviteUser} disabled={loading} className="btn-primary text-xs py-1.5 disabled:opacity-60">
