...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ client?: string }> }) {
  ...
  const [
    ...
    supabase.from('ads_integrations').select('id,client_id,platform,access_token,account_id,refresh_token,n8n_webhook_url,mcc_id,active,updated_at').eq('client_id', client.id).eq('active', true),
  ])
